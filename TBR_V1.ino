...
void processGamepad(ControllerPtr ctl) {
  ...
  if (ctl->axisY() <= -25) {
      ...
      motorSpeed=map(ctl->axisY(),-25,-508,150,1023);
      ledcWrite(channel1 , motorSpeed);
      ledcWrite(channel2 , 0);
      ledcWrite(channel3 , motorSpeed);
      ledcWrite(channel4 , 0);// code for when left joystick is pushed up
    }

  //== LEFT JOYSTICK - DOWN ==//
  if (ctl->axisY() >= 25) {
   motorSpeed=map(ctl->axisY(),-25,-508,150,1023);
      ledcWrite(channel2 , motorSpeed);
      ledcWrite(channel1 , 0);
      ledcWrite(channel4 , motorSpeed);
  ...
  if (ctl->axisX() >= 25) {
   motorSpeed=map(ctl->axisY(),-25,-508,150,1023);
      ...
      ledcWrite(channel2 , 0);
      ledcWrite(channel4 , motorSpeed);
      ledcWrite(channel3 , 0);// code for when left joystick is pushed right
  }
  ...
  if (ctl->axisY() > -25 && ctl->axisY() < 25 && ctl->axisX() > -25 && ctl->axisX() < 25) {
    ...
    ledcWrite(channel2 , 0);
    ledcWrite(channel4 , 0);
    ledcWrite(channel3 , 0);// code for when left joystick is at idle
  }
